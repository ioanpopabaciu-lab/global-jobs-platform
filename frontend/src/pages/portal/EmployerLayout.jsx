...
const navigation = [
  { name: 'Dashboard', href: '/portal/employer', icon: Home },
  { name: 'Profilul Companiei', href: '/portal/employer/profile', icon: Building2 },
  { name: 'Cereri de Personal', href: '/portal/employer/jobs', icon: Briefcase },
  { name: 'Proiecte Active', href: '/portal/employer/projects', icon: Users },
  { name: 'Facturi', href: '/portal/employer/invoices', icon: CreditCard },
  { name: 'Notificări', href: '/portal/employer/notifications', icon: Bell },
];
...
export default function EmployerLayout() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const response = await fetch(`${API_URL}/api/portal/employer/dashboard`, {
          credentials: 'include'
        });
        if (response.ok) {
          ...
          <div className="p-6 border-b">
            <Link to="/" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Portal Angajator</p>
              </div>
            </Link>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {dashboardData?.company_name || user?.name}
                </p>
            ...
            {dashboardData && !dashboardData.has_profile && (
              <div className="mt-3 p-2 bg-yellow-50 rounded-lg border border-yellow-200">
                <p className="text-xs text-yellow-800">
                  ⚠️ Completează profilul companiei pentru a crea cereri de personal
                </p>
              </div>
            )}
          </div>
